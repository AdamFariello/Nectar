...
public class DBInsert extends DBRetrieve {			
	...
	public static boolean insertIntoTableWithPrimaryKey(String table, ArrayList<Object> tableInputs) {			
		ArrayList<String> columnsOfTable = getColumnsFromTable_ArrStr(table);
		...
		String queryValues = "?, ".repeat(tableInputs.size()); 
		queryValues 	   = queryValues.substring(0, queryValues.length()-2);		
		String queryformat = "INSERT INTO %s VALUES (%s)";
		String query = String.format(queryformat, table, queryValues);
		
		System.out.println(columnsOfTable + "\n" + datatypesOfTable);
		
		return insertIntoTable(query, tableInputs, columnsOfTable, datatypesOfTable);
	}
	public static boolean insertIntoTableWithOutPrimaryKey(String table, ArrayList<Object> tableInputs) {
		ArrayList<String> columnsOfTable = getColumnsFromTable_ArrStr(table);
		...
		String queryColumns = columnsOfTable.toString().replace("[", "(").replace("]", ")");
		String queryString  = "INSERT INTO %s (%s) VALUES %s";
		String query = String.format(queryString, table, queryInserts, queryColumns);
		
		return insertIntoTable (query, tableInputs, columnsOfTable, datatypesOfTable);
	}
	private static boolean insertIntoTable (String query, ArrayList<Object> tableInputs, 
